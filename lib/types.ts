...
export const FREQUENCY_BANDS = [100, 125, 160, 200, 250, 315, 400, 500, 630, 800, 1000, 1250, 1600, 2000, 2500, 3150] as const
export const EXTENDED_FREQUENCY_BANDS = [50, 63, 80, ...FREQUENCY_BANDS, 4000, 5000] as const
...
export interface AereoParede {
  tipo: 'aereo_parede'
  npsSourceRoom: number | FrequencyData[]
  npsReceptionRoom: number | FrequencyData[]
}

export interface AereoFachada {
  tipo: 'aereo_fachada'
  npsExternal: number | FrequencyData[]
  npsInternal: number | FrequencyData[]
}
...
export const TEST_TYPES: TestTypeConfig[] = [
  {
    id: 'aereo_parede',
    name: 'Aéreo Parede',
    description: 'Isolamento a ruído aéreo entre ambientes internos separados por partição vertical',
    icon: 'wall',
    metrics: ['DnT', "R'", 'DnT,w', "R'w"],
    requiredFields: ['partitionArea', 'receptionVolume', 'npsSourceRoom', 'npsReceptionRoom']
  },
  {
    id: 'aereo_fachada',
    name: 'Aéreo Fachada',
    description: 'Isolamento a ruído aéreo de fachadas e elementos de vedação externa',
    icon: 'building',
    metrics: ['D2m,nT', 'D2m,n', "R'45°", 'D2m,nT,w', 'D2m,n,w'],
    requiredFields: ['partitionArea', 'receptionVolume', 'npsExternal', 'npsInternal']
  },
  {
    id: 'impacto_laje',
    name: 'Impacto Laje',
    description: 'Isolamento a ruído de impacto em pisos e lajes entre pavimentos',
    icon: 'layers',
